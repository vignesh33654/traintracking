...
export default function HornButton() {
  ...
  return (
    <button
      onClick={handleClick}
      className="hide-horn-mobile hover:cursor-pointer fixed left-4 md:left-auto md:right-4 bottom-4 md:bottom-18 w-14 h-14 md:w-10 md:h-10 z-50 flex flex-col justify-center items-center rounded-full bg-bg-0 border border-divider text-text-primary transition-none hover:bg-bg-1 focus-ring"
      aria-label="Play horn sound"
    >
      <HornIcon isDark={isDark} />
    </button>
  );
}
