...
const Card = forwardRef<HTMLDivElement, Props>(function Card(
  ...
  return (
    <Component
      className={cn(
        'ui-card fixed top-[16px] left-[16px] bg-panel-border shadow-card  rounded-[10px] w-full p-px',
        containerClassName,
      )}
      id={id}
    >
      <div
        className={cn(
          'card bg-panel-background w-100 relative z-1 flex h-full flex-col rounded-[9px] p-3 [&>*:first-child]:flex-1',
          className,
        )}
        ref={contentRef}
      >
